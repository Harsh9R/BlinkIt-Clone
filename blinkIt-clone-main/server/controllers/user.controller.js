...
export async function uploadAvatar(request, response) {
    try {
        ...
        if (!upload || !upload.secure_url) {
            console.error('Invalid upload response:', upload);
            return response.status(500).json({
...
export async function forgotPasswordController(request,response) {
    try {
        ...
        await sendEmail({
            sendTo : email, // Send to actual user's email
            subject : "Password Reset OTP - Ecom",
...
export async function googleAuth(request, response) {
    try {
        const { token, name, email, photoURL } = request.body;
